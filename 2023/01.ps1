...
foreach ($line in $lines) {
    ...
    For ($i = $LineLength; $i -ge 0; $i--) {
        If ($Line[$i - 1] -in @("1", "2", "3", "4", "5", "6", "7", "8", "9", "0")) {
            $LastNumber = $Line[$i - 1]
            break
        }
    }
    $values += [int]"$FirstNumber$LastNumber"
}
...
foreach ($line in $lines) {
    ...
    For ($i = 0; $i -lt $LineLength; $i++) {
        ...
        elseIf ($line[$i..($i+2)] -join '' -eq 'one') {
            $numbers+="1"
        }
        elseIf ($line[$i..($i+2)] -join '' -eq 'two') {
            $numbers+="2"
        }
        elseIf ($line[$i..($i+4)] -join '' -eq 'three') {
            $numbers+="3"
        }
        elseIf ($line[$i..($i+3)] -join '' -eq 'four') {
            $numbers+="4"
        }
        elseIf ($line[$i..($i+3)] -join '' -eq 'five') {
            $numbers+="5"
        }
        elseIf ($line[$i..($i+2)] -join '' -eq 'six') {
            $numbers+="6"
        }
        elseIf ($line[$i..($i+4)] -join '' -eq 'seven') {
            $numbers+="7"
        }
        elseIf ($line[$i..($i+4)] -join '' -eq 'eight') {
            $numbers+="8"
        }
        elseIf ($line[$i..($i+3)] -join '' -eq 'nine') {
            $numbers+="9"
        }
        elseIf ($line[$i..($i+3)] -join '' -eq 'zero') {
            $numbers+="0"
        }
    }
    $realValues += [int]"$($numbers[0])$($numbers[-1])"
}
$realValues | Measure-object -SUm
